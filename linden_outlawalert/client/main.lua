...
AddEventHandler('wf-alerts:clNotify', function(pData)
    if pData ~= nil then
        if not pData.length then pData.length = 8000 end
...
RegisterCommand('10-13', function(playerId, args, rawCommand)
    local name = ('%s %s'):format(firstname, lastname)
    local title = ('%s %s %s'):format(rank,"-", name)
    local data = {displayCode = '10-13', description = _U('officerdown'), isImportant = 1, recipientList = {'police', 'ambulance'}, length = '10000', infoM = 'fas fa-exclamation-circle', info = title, blipSprite = 653, blipScale = 1.1, blipColour = 1}
    local dispatchData = {dispatchData = data, caller = name, street = streetName, coords = playerCoords}
