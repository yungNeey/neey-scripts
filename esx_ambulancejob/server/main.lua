...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName)
	...
	if xPlayer.job.name ~= 'ambulance' then
		...
		return
	elseif (itemName ~= 'medikit' and itemName ~= 'bandage' and itemName ~= 'leki' and itemName ~= 'ggps' and itemName ~= 'radio' and itemName ~= 'stungun') then
		print(('esx_ambulancejob: %s attempted to spawn in an item!'):format(xPlayer.identifier))
		return
	end
	...
	if source == 0 then
		...
		if (xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'moderator' or xPlayer.group == 'support' or xPlayer.group == 'trialsupport') then
			if args[1] ~= nil then
				...
				TriggerClientEvent('FeedM:showNotification', source, '~g~Otrzymałeś revive')
			end
		else
