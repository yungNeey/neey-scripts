	...
	if not isUsing then
		if _type == 'bsmall' then
			if health <= 100 then 
				...
				exports['SpaceRP']:DrawProcent(300, function()
					Citizen.InvokeNative(0x6B76DC1F3AE6E6A3, playerPed, newHealth)
...
function StartDistressSignal()
	Citizen.CreateThread(function()
		...
		while timer > 0 and IsDead do
			...
			EndTextCommandDisplayText(0.420, 0.905)

			if IsControlPressed(0, Keys['G']) then
				TriggerServerEvent('esx_ambulancejob:odbierzsygnal')
...
RegisterNetEvent('esx_ambulancejob:sygnal')
AddEventHandler('esx_ambulancejob:sygnal', function()
	Citizen.Wait(100)
	SendDistressSignalA()
	ESX.TriggerServerCallback("esx_scoreboard:getConnectedCops", function(ZlomusPlayers)
		if ZlomusPlayers then
			local ems = ZlomusPlayers['ambulance']
			if ems < 2 then
				SendDistressSignalP()
			end
		end
...
function RemoveItemsAfterRPDeath()
	CreateThread(function()
		...
		ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()
			...
			RespawnPed(PlayerPedId(), Config.RespawnPlace)
...
function OpenMobileAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions',
	{
		...
		elseif data.current.value == 'Kajdanki' then
			menu.close()
			exports['space_handcuffs']:HandcuffsAction()
				...
				if not IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
					...
					if data.current.value == 'repair' then
						if(not IsPedInAnyVehicle(playerPed)) then
							TriggerEvent('esx_mechanicjob:onFixkitFree')
						end
					elseif data.current.value == 'hijack' then
						if(not IsPedInAnyVehicle(playerPed)) then
							TriggerEvent('odblokujpol')
						end
...
function OpenPharmacyMenu()
	...
	'default', GetCurrentResourceName(), 'pharmacy',
	{
		...
		elements = {
			...
			{label = _U('pharmacy_take') .. ' ' .. "Gps", value = 'ggps', count = 1},
...
AddEventHandler('esx_ambulancejob:revive', function(notBlock)
	...
	if IsDead and not notBlock then
		...
		blockShooting = GetGameTimer() + (10 * 60000)
	end
	TriggerServerEvent('esx_ambulancejob:setDeathStatus', 0)
...
AddEventHandler('esx_ambulancejob:revive', function(hospital)
  ...
  Citizen.CreateThread(function()
	...
	DoScreenFadeIn(800)
	IsDead = false
	ESX.ShowNotification("~r~Nie mozesz uzyc broni przez 1min")
	aimwlonczony = true	
  end)
end)
Citizen.CreateThread(function()
		...
		while true do
			...
			while startcout do
				Citizen.Wait(60000)
					...
					startcout = false
					ESX.ShowNotification("~g~Odzyskales sily na trzymanie broni")
			end
		end
end)